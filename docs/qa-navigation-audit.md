...
| 6 | Active-state désynchronisé (AppSidebar vs sidenav interne) | ✅ OK | Sidebars internes supprimées. Un seul mécanisme : `routerLinkActive="sidebar__nav-link--active"` dans `app-sidebar.component.html`. |
...
| 9 | Style sidebar non unifié (emojis vs icônes vectorielles, typographie et états actif divergents entre nav commune et nav contextuelle) | ✅ OK — **étendu au rôle Administrateur le 2026-08-08** | Bloc contextuel migré vers les mêmes icônes SVG Heroicons v2 outline que la nav commune (`sidebar__icon` 20 × 20 px, `stroke-width="1.5"`). Passe initiale : classes `__ctx-link` / `__ctx-label` alignées visuellement sur `__link` / `__label` par duplication de règles CSS. **2026-08-08 :** duplication éliminée — `__link`/`__ctx-link` et `__label`/`__ctx-label` fusionnés en une seule paire de classes factorisées `.sidebar__nav-link` / `.sidebar__nav-label`, utilisées à la fois par la nav commune (Dashboard/Dossiers/Clients) et par tous les blocs contextuels (`ADMINISTRATION`, `Espace Superviseur`, `Espace Manager`). Le bloc `ADMINISTRATION`, qui n'avait pas été vérifié explicitement lors de la première passe, est désormais visuellement indiscernable du bloc du haut (icônes, typographie, état actif/hover, espacement). Aucune régression sur les 3 autres blocs contextuels, qui consomment la même classe factorisée. `RoleMenuEntry.icon` typé `SidebarIconName` (union de 15 noms Heroicons) ; paths centralisés dans `sidebar-icons.ts` (couche `shared/data-access`). |
| 10 | Double état actif "Paramétrage" + "Référentiels" simultanément en surbrillance dans le bloc `ADMINISTRATION` sur `/parametrages/referentiels` | ✅ OK — **corrigé le 2026-08-08** | Cause : `MENU_CONFIG.ADMIN` contient un item "Paramétrage" cliquable pointant vers `/parametrages` (qui redirige lui-même vers `/parametrages/referentiels`, `settings.routes.ts` — pas de page propre). `AppSidebarComponent.linkActiveOptions()` appliquait `paths: 'subset'` à tous les items sans `queryParams`, donc `/parametrages` matchait par préfixe dès qu'une sous-route (`referentiels`, `regles-workflows`, `imports`, `audit`) était active — les deux items s'allumaient en même temps. C'est le seul bloc contextuel reproduisant ce pattern parent/enfant (vérifié : `SUPERVISOR`, `MANAGER` et `ADMIN` hors "Paramétrage" n'ont que des routes sœurs, jamais l'une préfixe de l'autre). Correctif : "Paramétrage" reste un lien cliquable (item de tête du chapitre, atterrissage sur `/parametrages` → redirection immédiate vers Référentiels), mais un nouveau champ `RoleMenuEntry.exactMatch?: boolean` force `paths: 'exact'` dans `linkActiveOptions()` pour cet item — il ne s'active plus que sur `/parametrages` strictement, jamais sur ses sous-routes. Testé par 2 nouveaux specs routés (`app-sidebar.component.spec.ts`, describe `AppSidebarComponent — active state (ADMIN, /parametrages/*)`) qui naviguent réellement vers `/parametrages/referentiels` puis `/parametrages/audit` et vérifient qu'un seul item actif est présent dans le bloc contextuel à chaque fois. **Note (anomalie #11 ci-dessous) :** depuis la création du composant `SideNavPanelComponent`, la classe active portée par les items du bloc contextuel est devenue `.snp__item--active` (au lieu de `.sidebar__nav-link--active` mentionné ci-dessus, qui reste correct pour la nav commune Dashboard/Dossiers/Clients) ; le comportement `exactMatch` décrit n'est pas affecté, seule la classe CSS a changé de porteur. |
| 11 | Icônes emoji dans les panneaux de navigation secondaire `DOMAINES` (Référentiels) et `PACKAGES DMN` / `DIAGRAMMES` (Règles & Workflows), non alignées sur la bibliothèque vectorielle de la sidebar | ✅ OK — **corrigé le 2026-08-08** | Cause : `ParametragesReferentielsComponent.DOMAINS` et `ParametragesReglesWorkflowsComponent.DECISION_TYPES` / `WORKFLOW_DEFS` stockaient un `icon: string` contenant un emoji brut (🏦 📋 📌 ⚡ 🔄 📡 📦 🎯 🔺 🗂️ 📅 ⚖️), rendu tel quel dans le template (`{{ d.icon }}` / `{{ dt.icon }}`, y compris concaténé au titre `{{ wf.icon }} {{ wf.label }}`) — jamais passé par la bibliothèque Heroicons utilisée dans `AppSidebar`. Correctif : nouveau registre partagé `shared/data-access/icon-registry.ts` (`IconName` / `ICON_PATHS`, remplace `sidebar-icons.ts`, étendu à 26 icônes dont `building-library`, `tag`, `flag`, `bolt`, `arrow-path`, `signal`, `cube`, `chart-pie`, `calendar-days`, `home`, `folder`) consommé par un nouveau composant `mc-icon` (`shared/ui/icon`). Les trois champs `icon` sont maintenant typés `IconName` (compilation échoue si une chaîne emploi n'appartient pas au registre — impossible de repasser silencieusement à une chaîne emoji). Réutilisation sémantique : "Processus contentieux" → `scale` (même icône que "Contentieux" dans le menu Superviseur) ; "Règles d'escalade" et "Flux d'escalade" → `exclamation-triangle` (même icône que "Escalades" côté Superviseur) ; "Cycle de vie dossier" → `folder` (même icône que "Dossiers" en nav commune). **Composant partagé `SideNavPanelComponent` créé (point 5 de la demande) :** `shared/ui/side-nav-panel/` — panneau titre + liste d'items (icône vectorielle + libellé + badge de comptage optionnel + état actif), deux modes (`route` → lien routé avec `routerLinkActive`, sans `route` → bouton local piloté par `[activeKey]`/`(itemSelected)`), variante `light` (défaut, panneaux settings) / `dark` (sidebar navy). **Les 3 emplacements dupliqués ont été migrés vers ce composant unique :** bloc `ADMINISTRATION`/`Espace Superviseur`/`Espace Manager` de l'`AppSidebar` (`variant="dark"`), panneau `DOMAINES` de Référentiels, panneaux `PACKAGES DMN` et `DIAGRAMMES` de Règles & Workflows. Le CSS dupliqué (`.ref__nav-item` et `.rw__nav-item`, littéralement identiques avant correctif) a été supprimé des deux composants settings ; seul `SideNavPanelComponent` porte désormais ce style. Badge de comptage : nouveau composant partagé `mc-count-badge` (`shared/ui/count-badge`), utilisé par `PACKAGES DMN` (déjà présent) et ajouté à `DOMAINES` (`navItems` computed sur `ParametragesReferentielsComponent`, compte les valeurs référentielles par domaine, ex. "Créanciers · 2") — les deux panneaux utilisent désormais le même composant, plus de style dupliqué. Vérifié : `ng build` et `ng test` passent sans erreur de compilation de template après migration. |

### Bilan

**11/11 anomalies corrigées.** Aucun KO résiduel. Les prompts fonctionnels suivants peuvent reprendre sans blocage de navigation ni incohérence visuelle de la sidebar ou des panneaux de navigation secondaire.
